...
function Sidebar({ color, image, routes }) {
  ...
  const activeRoute = (routeName) => {
    ...
    const routePath = routeName.replace('/admin/', '');
    console.log('Checking route:', { currentPath, adminPath, routePath, routeName });
    return adminPath === routePath ? "active" : "";
  };
  
  const handleNavClick = (route) => {
    console.log('Navigating to:', route.path);
    console.log('Full route object:', route);
    console.log('Full navigation path:', route.layout + "/" + route.path);
  };
