...
import { getGoal, setGoal, updateGoal, deleteGoal, addAmount, completeGoal } from '../api/api';
...
function Maps() {
  ...
  const notifyFailure = (message = "An Error Occurred") => toast.error(message, {
    position: "top-right",
    autoClose: 5000,
    hideProgressBar: false,
    closeOnClick: true,
  ...
  const fetchGoals = async () => {
    await getGoal().then((res) => {
      ...
      // Handle both old format (array) and new paginated format (object with goals array)
      if (Array.isArray(res.data)) {
        setAllGoals(res.data);
      } else if (res.data && res.data.goals) {
  ...
  const createNewGoal = async (e) => {
    ...
    // Validate inputs
    if (!newGoal || !newtargetAmount || !newcurrentAmount) {
      notifyFailure("Please fill all required fields")
      return
    }
    
    // Convert to numbers
    const targetAmt = parseFloat(newtargetAmount)
    const currentAmt = parseFloat(newcurrentAmount)
    
    if (isNaN(targetAmt) || targetAmt <= 0) {
      notifyFailure("Target amount must be a positive number")
      return
    }
    
    if (isNaN(currentAmt) || currentAmt < 0) {
      notifyFailure("Current amount must be a non-negative number")
      return
    }
    
    if (currentAmt > targetAmt) {
      notifyFailure("Current amount cannot be greater than target amount")
      return
    }
    ...
    await setGoal(goalData)
      .then(() => {
        notifySuccess("Successfully Added New Goal")
        // Clear form
        setNewGoal("")
        setNewtargetAmount("")
        setNewcurrentAmount("")
      ...
      .catch(e => {
        ...
        notifyFailure(e.response?.data?.message || "Failed to create goal. Please check your input.")
      })
  }
